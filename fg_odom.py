...
odom_poses = []
for l in _load_text("direct_poses.txt"):
    odom_poses.append(
        jnp.array([float(x) for x in l.split(" ")]).reshape((3, 4)))
odom_poses = jnp.array(odom_poses)
print(f"{odom_poses.shape = }")
odom_positions = jnp.array([x[:, -1] for x in odom_poses])
...
def _from_matrix(m):
    rot = gtsam.Rot3(*m[:, :3].flatten())
    pos = gtsam.Point3(*m[:, -1].flatten())
    return gtsam.Pose3(rot, pos)


prior_mean = _from_matrix(odom_poses[0])
...
for i in range(start_idx, start_idx + num_frames):
    ...
    current_pose_estimate = _from_matrix(odom_poses[i])
...
optimizer = gtsam.LevenbergMarquardtOptimizer(new_factors, new_values)
